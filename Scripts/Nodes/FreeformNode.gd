...
func get_prefab(type: String):
	...
	var elementData = Globals.elementConfigs[fieldData.type]
	if Functions.is_app():
		return load(Functions.os_path_convert(Globals.resDataPath + "Custom/FreeformElements/Prefabs/" + type, false)).instance()
	else:
		return load(Globals.resDataPath + "Custom/FreeformElements/Prefabs/" + type).instance()
