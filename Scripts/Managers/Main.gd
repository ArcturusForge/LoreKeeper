...
# Managers
onready var plugin_manager = $Managers/PluginManager
...
func _ready():
	...
	var optionsBtn = popup_manager.get_popup_data("optionMenu")
	optionsBtn.register_entity(my_id, self, "on_option_selected")
	optionsBtn.add_separator(my_id, "Options")
	# Save
	optionsBtn.add_option(my_id, "Save", KEY_S)
	# Save As
	optionsBtn.add_option(my_id, "Save As", KEY_S, true)
	optionsBtn.add_separator(my_id)
	# Load
	optionsBtn.add_option(my_id, "Load", KEY_L)
	# New
	optionsBtn.add_option(my_id, "New", KEY_N)
	optionsBtn.add_separator(my_id)
	# Plugins
	optionsBtn.add_option(my_id, "Plugins", KEY_P, true)
	...
	var elements = Functions.get_all_files(Globals.resDataPath + Globals.elementsPath, Globals.elementExtension)
	for element in elements:
		load_element_config(element)
	
	# Locate and iterate over every window type
	var windows = Functions.get_all_files(Globals.resDataPath + Globals.windowsPath, Globals.windowExtension)
	for window in windows:
		load_window_config(window)
	
	# Locate and iterate over every style type
	var styles = Functions.get_all_files(Globals.resDataPath + Globals.stylesPath, Globals.styleExtension)
	for style in styles:
		load_style_config(style)
	
	plugin_manager._detect_plugins()
	
	if not Functions.open_with_cmd(self, "load_existing_session"):
...
func view_entry(index: int):
	...
	for selector in windowConfig.fields:
		addAttributableBtn.get_popup().add_icon_item(Functions.load_image(Globals.resDataPath + Globals.iconsPath + selector.icon), selector.prompt)
	pass
...
func on_option_selected(option:String):
	match option:
		"Save": # Save
			if Session.savePath == "":
				search_manager.search_to_save(self, "_on_SaveSessionDialog_file_selected")
			else:
				Session.quick_save()
		"Save As": # Save As
			search_manager.search_to_save(self, "_on_SaveSessionDialog_file_selected")
		#2: Seperator 
		"Load": # Load
			search_manager.search_for_session(self, "_on_SessionFileDialog_file_selected")
		"New": # New
			_on_NewButton_pressed()
		"Plugins": # Plugins
			plugin_manager._generate_plugin_menu()
