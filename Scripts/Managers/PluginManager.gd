...
func _detect_plugins():
	...
	var detected = Functions.get_all_files(Globals.resDataPath + Globals.pluginsPath, Globals.pluginExtension)
	detected = Functions.get_all_files(Globals.userDataPath + Globals.pluginsPath, Globals.pluginExtension, true, detected)
...
func _load_plugin(id: int, saveCache = true):
	...
	var driver = Functions.get_from_prefab(plugin.path + "/" + plugin.config.driver)
